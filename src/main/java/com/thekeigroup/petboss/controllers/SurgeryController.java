...
public class SurgeryController implements Serializable {
    ...
    public void saveTreatment(){
        ...
        try {
            if(null == selectedPet){
                Msg.genericError("Please select Pet for surgery");
                return;
            }
            if(null == selectedDiagnosis)
            {
                Msg.genericError("Please select type of surgery");
                return;
            }
    ...
    {
//        treatmentsList = commonService.petTreatmentHistoryByDiagnosis(selectedPet, DiagnosisCategory.SURGERIES);
